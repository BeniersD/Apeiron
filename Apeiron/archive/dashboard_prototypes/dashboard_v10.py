...
# --- CONFIGURATIE ---
st.set_page_config(page_title="Apeiron v9.4 | Full Detail & Entropy", layout="wide", page_icon="🧠")
...
# --- UI START ---
st.title("🧠 Apeiron v9.4: Deep Ontological Monitor")
data = get_state()
...
if data:
    ...
    with c_right:
        # --- NIEUW: ENTROPY METER ---
        st.write("### 🧬 Systeem Gezondheid (Entropy)")
        entropy = data.get('entropy', 0.5)
        
        # Kleur bepalen op basis van entropy niveau
        if entropy < 0.3:
            st.error(f"⚠️ **Kritiek Lage Entropy: {entropy:.2f}**")
            st.info("De AI bevindt zich in een herhalings-lus. Diversiteits-injectie aanbevolen.")
        elif entropy > 0.7:
            st.success(f"🚀 **Hoge Entropy: {entropy:.2f}**")
            st.write("De AI ontdekt actief nieuwe, diverse informatiepaden.")
        else:
            st.warning(f"⚖️ **Stabiele Entropy: {entropy:.2f}**")
        ...
        if inv:
            for i in inv: st.write(f"✅ `{i}`")
        else:
            st.info("Geen stabiele invarianten.")
...
# --- CHAT SECTIE ---
st.divider()
st.subheader("💬 De Apeiron Chat (Deep Memory Retrieval)")
if "messages" not in st.session_state: st.session_state.messages = []

for m in st.session_state.messages:
    with st.chat_message(m["role"]): st.markdown(m["content"])

if prompt := st.chat_input("Bevel de Apeiron om deze stappen te synthetiseren..."):
        ...
        if results['documents'][0]:
            with st.chat_message("assistant"):
                for i, doc in enumerate(results['documents'][0]):
                    title = results['metadatas'][0][i].get('title', 'Onbekend Concept')
                    with st.expander(f"Bron {i+1}: {title}", expanded=(i==0)):
                        st.markdown(doc)
                
                ans = "De bovenstaande concepten vormen een coherente keten in je huidige ontologie."
                st.markdown(ans)
                st.session_state.messages.append({"role": "assistant", "content": ans})
        else:
            ans = "Geen directe correlaties gevonden."
            st.warning(ans)
            st.session_state.messages.append({"role": "assistant", "content": ans})
