...
if prompt := st.chat_input("Bevel de Nexus om deze stappen te synthetiseren..."):
    ...
    try:
        memory = init_db()
        # We halen nu 3 resultaten op voor een breder overzicht
        ...
        if results['documents'][0]:
            with st.chat_message("assistant"):
                for i, doc in enumerate(results['documents'][0]):
                    ...
                    # Gebruik expanders voor lange teksten om het dashboard schoon te houden
                    with st.expander(f"Bron {i+1}: {title}", expanded=(i==0)):
                        st.markdown(doc) # Geen [:600] meer, toon alles!
                
                ans = "De bovenstaande concepten vormen een coherente keten in je huidige ontologie."
                st.session_state.messages.append({"role": "assistant", "content": ans})
        else:
            st.warning("Geen directe correlaties gevonden.")
