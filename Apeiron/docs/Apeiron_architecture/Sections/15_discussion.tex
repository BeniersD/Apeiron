...
\begin{itemize}
    ...
    \item \textbf{Mathematical idealisation versus computational tractability.} Several core constructs---Kolmogorov complexity, fixed-point theorems for operator families, persistent homology over unbounded state spaces---are mathematically well-defined but computationally intractable in their pure form. The framework addresses this by distinguishing between the ideal formal specification and the proxy-based reference implementation, and by embedding explicit error monitoring in the MetaSpecification and the coherence tensor. Nevertheless, the long-term convergence of the implemented system to the ideal model remains an open theoretical question, and the empirical validation programme is designed to probe this gap progressively.
...
\noindent\textbf{Measuring non‑anthropocentricity.}
An open challenge is the operational quantification of the degree to which the system's knowledge diverges from human categories. Promising directions include: (i) information‑theoretic divergences between discovered clusters and human label distributions (mutual information, KL‑divergence); (ii) structural distance measures between generated ontologies and human ontologies via graph‑distance metrics; (iii) behavioral tests in which the system solves tasks using representations that systematically differ from human concepts. We regard the development and validation of such metrics as a core component of the research programme's next phase.
