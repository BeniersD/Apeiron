...
% =======================================================
% EMPIRICAL RESOURCE FOOTPRINT – definitief met jouw JSON
% =======================================================
\subsection{Empirical Resource Footprint of the Cluster~I Implementation}
\label{subsec:empirical_resources}

To ground the complexity analysis, we profiled the reference
implementation on a commodity workstation:
\emph{Windows~10, Intel\textsuperscript{\textregistered} Core\textsuperscript{\texttrademark} i7-8750H
(6 physical / 12 logical cores, 2.2\,GHz base), 32\,GB DDR4‑2666 RAM,
no GPU used}.
%
The atomicity verification of the 47‑observable benchmark
(Table~\ref{tab:examples}) completed in \texttt{0.01}\,s with a peak
memory of \texttt{0.3}\,MB.
%
The full unsupervised clustering pipeline---hypergraph construction,
Louvain community detection, and counterfactual ablation for all five
visual datasets plus the CIFAR‑10 Omega variant
(Table~\ref{tab:all_experiments})---required \texttt{440.7}\,s
($\sim$7.3\,min) and \texttt{2038.3}\,MB ($\sim$1.99\,GB) RAM.
%
The complete profiling script and the resulting hardware‑performance
logs are included in the repository
(\texttt{profile\_cluster1.py} and
\texttt{profile\_cluster1\_results.json}).