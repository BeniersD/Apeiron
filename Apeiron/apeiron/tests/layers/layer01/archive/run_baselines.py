...
# Direct open a debug log file
debug_log = open("debug_log.txt", "w", encoding="utf-8")

def log(msg):
    """Write message to both terminal and debug log."""
    print(msg, flush=True)
    debug_log.write(msg + "\n")
    debug_log.flush()

log("=" * 60)
log("APERION BASELINE EXPERIMENT (DEBUG VERSION)")
log("=" * 60)
...
try:
    ...
    log("Libraries imported OK")

    # ------------------------------------------------------------
    # 1. Load digits dataset
    # ------------------------------------------------------------
    log("Step 1: Loading digits dataset (8x8)...")
    digits = load_digits()
    X = digits.data / 16.0
    y = digits.target
    # Use a small subset for speed and safety
    n_samples = 500
    X = X[:n_samples]
    y = y[:n_samples]
    n_features = X.shape[1]
    log(f"  Using subset: {X.shape[0]} samples, {n_features} features, {len(np.unique(y))} classes")

    # ------------------------------------------------------------
    # 2. Compute co-activation graph
    # ------------------------------------------------------------
    ...
    def ablation_study(X, y, cluster_labels, n_runs=2):
    ...
    kmeans_ablation = ablation_study(X, y, kmeans_labels, n_runs=2)
    ...
    spectral_ablation = ablation_study(X, y, spectral_labels, n_runs=2)
...
"""
    ...
    with open("baseline_table.tex", "w", encoding="utf-8") as f:
        f.write(latex_table)
    log("Table also written to baseline_table.tex")
    ...
    log("Error details written to debug_log.txt")

finally:
    debug_log.close()
    log("\nScript finished. Check debug_log.txt and baseline_table.tex")