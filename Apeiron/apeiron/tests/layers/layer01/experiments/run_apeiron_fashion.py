# -*- coding: utf-8 -*-
"""
Apeiron Layer 1-2-3 toegepast op Fashion-MNIST (28x28).
"""
...
debug_log = open("apeiron_fashion_log.txt", "w", encoding="utf-8")
...
log("=" * 60)
log("APEIRON ON FASHION-MNIST (28x28)")
log("=" * 60)

try:
    log("Step 1: Loading Fashion-MNIST...")
    X, y = fetch_openml('Fashion-MNIST', version=1, return_X_y=True, as_frame=False, parser='auto')
    X = X / 255.0
    y = y.astype(int)
    # Gebruik subset voor snelheid (optioneel, pas aan indien nodig)
    n_samples = 5000  # pas aan naar wens (max 70000)
    X = X[:n_samples]
    y = y[:n_samples]
    ...
    # Genereer LaTeX-tabel (alleen Apeiron, of voeg baselines toe als je die ook draait)
    latex_table = f"""
\\begin{{table}}[htbp]
\\centering
\\caption{{Apeiron performance on Fashion-MNIST (subset of {n_samples} samples).}}
...
"""
    ...
    with open("apeiron_fashion_table.tex", "w", encoding="utf-8") as f:
        f.write(latex_table)
    log("Table written to apeiron_fashion_table.tex")
