# -*- coding: utf-8 -*-
"""
Apeiron Layer 1-2-3 applied to Kuzushiji-MNIST (28x28).
"""
...
debug_log = open("apeiron_kmnist_log.txt", "w", encoding="utf-8")
...
log("=" * 60)
log("APEIRON ON KUZUSHIJI-MNIST (28x28)")
log("=" * 60)

try:
    log("Step 1: Loading Kuzushiji-MNIST...")
    X, y = fetch_openml('Kuzushiji-MNIST', version=1, return_X_y=True, as_frame=False, parser='auto')
    X = X / 255.0
    y = y.astype(int)
    # Subset voor snelheid (totale dataset is 70k, wij gebruiken 5k)
    n_samples = 5000
    X = X[:n_samples]
    y = y[:n_samples]
...
\\begin{{table}}[htbp]
\\centering
\\caption{{Apeiron performance on Kuzushiji-MNIST (subset of {n_samples} samples).}}
...
"""
    ...
    with open("apeiron_kmnist_table.tex", "w", encoding="utf-8") as f:
        f.write(latex_table)
    log("Table written to apeiron_kmnist_table.tex")
