...
Elke methode (k-means, spectral, Apeiron) wordt op dezelfde data geëvalueerd.
De tabel wordt weggeschreven naar `tables/all_experiments.tex`.
"""
...
apeiron_dir = os.path.join(script_dir, 'apeiron')
if os.path.exists(apeiron_dir):
    ...
    # Als Apeiron niet als package beschikbaar is, wordt de clustering overgeslagen.
    pass
...
def load_data(name, cfg):
    if cfg['type'] == 'sklearn':
        ...
        if cfg['type'] == 'torchvision_omega':
           ...
           # Flatten de grijswaarde-afbeeldingen naar (N, H*W)
...
def extract_omega_features(images):
    ...
    def build_gabor_filters(kernel_size=15, n_orientations=8, n_frequencies=2):
        ...
        return filters
    gabor_filters = build_gabor_filters(15, 8, 2)  # 16 filters
    N, H, W = images.shape
    gabor_feats = []
    for img in images:
        feats = []
    ...
    X_gabor = np.array(gabor_feats, dtype=np.float32)  # 256 features
    # Random CNN
    ...
    with torch.no_grad():
        cnn_feats = cnn(X_tensor).cpu().numpy().reshape(N, -1)  # 512 features
    X_combined = np.concatenate([X_gabor, cnn_feats], axis=1)
    # Verwijder constante kolommen
...
def main():
    ...
    for name, cfg in DATASETS.items():
        ...
        # Apeiron
        try:
            ape_mod, ape_drop, ape_std = run_method(X, y, method='apeiron')
        except Exception:
            ape_mod, ape_drop, ape_std = None, None, None
        # Baselines (alleen voor datasets zonder Omega, anders oneerlijk)
        if 'omega' not in name:
            km_mod, km_drop, km_std = run_method(X, y, method='kmeans')
            sp_mod, sp_drop, sp_std = run_method(X, y, method='spectral')
        else:
            km_mod = km_drop = km_std = sp_mod = sp_drop = sp_std = None
        results[name] = {
            'kmeans': (km_mod, km_drop, km_std),
            'spectral': (sp_mod, sp_drop, sp_std),
            'apeiron': (ape_mod, ape_drop, ape_std),
        }

    # LaTeX-tabel genereren
    lines = []
