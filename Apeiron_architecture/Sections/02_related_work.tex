...
The need for verifiable AI has spurred developments in \textit{neuro-symbolic verification} and \textit{certified learning}. Systems like \textit{AlphaVerus} \cite{wang2023} and \textit{VeriDeep} \cite{chen2024} provide guarantees for specific neural network properties, but they operate post-hoc on trained models. In contrast, the \Apeiron{} Framework embeds formal verification at the architectural level: atomicity is not a property to be checked after training but a constitutive condition of the irreducible observables (Layer~1). The \texttt{SelfProvingAtomicity} module provides on-the-fly theorem proving for atomicity claims.
...
The \Apeiron{} Framework follows in this tradition. Like these works, it aims to establish a coherent conceptual and formal foundation---in this case, for non-anthropocentric knowledge genesis---upon which future theoretical and empirical work can build. The present paper fully specifies and partially implements the foundational Cluster I, while Clusters II--IV are presented as detailed architectural specifications that define the trajectory for the research programme.
