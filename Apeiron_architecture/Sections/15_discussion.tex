...
In the landscape of neuro-symbolic AI and formal verification (e.g., AlphaVerus \cite{wang2023}, VeriDeep \cite{chen2024}), \Apeiron{} differs by embedding verification at the architectural level rather than applying it post-hoc to trained models. The \texttt{SelfProvingAtomicity} module provides on-the-fly theorem proving for atomicity claims, a capability that remains rare in mainstream AI systems.
...
The current reference implementation is restricted to Layer~1, the irreducible substrate. While this layer has been formally specified, empirically validated (Section~\ref{sec:validation}), and analyzed for computational complexity (Section~\ref{sec:complexity}), the higher layers remain theoretical constructs awaiting implementation. The primary challenges for extending the implementation to Layers 2--17 include:
...
\begin{itemize}
    ...
    \item \textbf{Empirical validation of higher-layer hypotheses:} The falsifiable predictions outlined in Section~\ref{sec:validation} for Layers 2 and 13 provide a roadmap, but realizing the simulation environments needed to test them demands significant development effort.
...
The immediate priority is the implementation of Layer~2 (Relational Emergence) and Layer~3 (Functional Emergence), enabling end-to-end validation of the first cluster. Concurrently, the \texttt{DensityField} and \texttt{UltimateObservable} classes will be extended to support the meta-learning operators of Layers 5--6, providing a substrate for empirical investigation of RQ5--RQ7.

In the medium term, the development of a categorical translation framework for Layers 9--12 will be pursued, leveraging applied category theory \cite{fong2019} and recent advances in functorial data migration. The ethical and governance primitives of Layers 15--17 will be elaborated into a formal specification suitable for legal and regulatory review.
