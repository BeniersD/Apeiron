...
Table~\ref{tab:all_datasets} summarizes the performance of Apeiron, $k$-means, and spectral clustering across five datasets. Apeiron consistently achieves the highest modularity on every dataset, with scores ranging from $Q = 0.46$ (Digits) to $Q = 0.36$ (CIFAR-10). The top cluster ablation impact is either the highest or statistically tied with the best baseline on all datasets.
...
\textbf{CIFAR-10.} Apeiron attains $Q = 0.43$, exceeding the target of $0.40$ and substantially outperforming the raw‑pixel variant ($Q = 0.23$). The ablation drop of $13.7\%$ indicates that the discovered clusters capture functionally relevant patterns in natural images. This result highlights the framework's ability to incorporate richer input representations without altering its foundational principles. The success on CIFAR‑10, a dataset far more complex than MNIST, validates the scalability of the relational hypergraph approach and suggests that further gains may be achievable with deeper feature hierarchies.
...
\textbf{Formal Verification Scope.} The formal proofs provided in this work apply strictly to canonical, noise-free observables (e.g., the integer~1). For high-dimensional, continuous data such as image pixels, the multi-axial atomicity criteria are evaluated heuristically. While the implementation caches results and uses sampling to bound complexity, a rigorous extension of the formal guarantees to these domains remains an open theoretical challenge. The current validation therefore demonstrates \emph{internal consistency} of the framework on its foundational objects, not a proof that every pixel is atomically irreducible in the strict mathematical sense.
% ============================================
